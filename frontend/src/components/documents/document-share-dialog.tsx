...
import { createDocumentShareLink, revokeDocumentShareLink } from '@/services/document.service';
...
export function DocumentShareDialog({
  ...
  const formattedExpiry = useMemo(() => {
    ...
    try {
      return formatDistanceToNow(parseISO(shareLink.expiresAt), { addSuffix: true });
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Tạo đường dẫn tạm thời để cấp quyền xem tài liệu cho người khác mà không cần đăng nhập.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-2 rounded-md border p-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Ở lần tạo tiếp theo, hệ thống sẽ sinh mã chia sẻ mới và vô hiệu hóa đường dẫn cũ.
              </p>
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
          ...
          <Button type="button" disabled={isGenerateDisabled} onClick={handleCreateShareLink}>
