...
export function AdminDashboardPage(): ReactElement {
  const [activeTab, setActiveTab] = useState<'moderation' | 'private'>(
    'moderation',
  );
  ...
  const [privateDocumentsData, setPrivateDocumentsData] =
    useState<PrivateDocumentsResponse | null>(null);
  ...
  const privateDocuments = useMemo(
    () => privateDocumentsData?.documents ?? [],
    [privateDocumentsData?.documents],
  );
  ...
  const filteredPrivateDocuments = useMemo(() => {
    if (!privateSearchQuery.trim()) return privateDocuments;
    const query = privateSearchQuery.toLowerCase();
    return privateDocuments.filter(
      doc =>
        doc.title.toLowerCase().includes(query) ||
        doc.description?.toLowerCase().includes(query) ||
        doc.uploader.username?.toLowerCase().includes(query),
    );
  }, [privateDocuments, privateSearchQuery]);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Tabs
        value={activeTab}
        onValueChange={value => setActiveTab(value as 'moderation' | 'private')}
      >
            ...
            <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
              <DialogContent className="h-[85vh] !max-w-[75%]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Kiểm tra nội dung và đưa ra quyết định duyệt/từ chối tài
                    liệu.
                  </DialogDescription>
                </DialogHeader>
                      ...
                      <Tabs defaultValue="info" className="w-full">
                            ...
                            <Card>
                              <CardContent className="flex items-center justify-between p-4">
                                <div className="space-y-0.5">
                                  ...
                                  <p className="text-muted-foreground text-xs">
                                    Nếu tắt, tài liệu sẽ được duyệt nhưng vẫn
                                    riêng tư
                                  </p>
                                </div>
                                <Switch
                        ...
                        <TabsContent value="ai" className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleGenerateAi(selectedDocument.id)
                              }
                              disabled={aiLoading}
                            >
                              {aiLoading ? (
                            ...
                            <div className="space-y-4">
                              ...
                              <Card>
                                <CardContent className="space-y-4 p-4">
                                  ...
                                  {selectedDocument.aiAnalysis.keyPoints
                                    ?.length ? (
                                    <div>
                                      <h4 className="mb-2 font-semibold">
                                        Điểm chính
                        ...
                        <TabsContent value="similarity" className="space-y-4">
                          ...
                          {_similarityLoading ? (
                            ...
                            <div className="space-y-3">
                              <p className="text-muted-foreground text-sm">
                                Tìm thấy {similarityWarnings.length} tài liệu có
                                nội dung tương đồng. Vui lòng xem xét và đánh
                                dấu là trùng lặp hoặc khác biệt.
                              </p>
                            ...
                            <Card>
                              <CardContent className="flex flex-col items-center justify-center gap-4 p-12 text-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-muted-foreground text-sm">
                                    Tài liệu này không có nội dung tương đồng
                                    với các tài liệu khác trong hệ thống.
                                  </p>
                                </div>
                              </CardContent>
                            </Card>
        ...
        <TabsContent value="private" className="space-y-6">
          <Card>
            ...
            {/* Pagination */}
            {privateDocumentsData && privateDocumentsData.total > 10 && (
              <div className="flex items-center justify-between border-t p-4">
                <p className="text-muted-foreground text-sm">
                  Trang {privatePage} /{' '}
                  {Math.ceil(privateDocumentsData.total / 10)}
                </p>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
